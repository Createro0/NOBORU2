...
-- Load built-in parsers from app0:assets/parsers/
if doesDirExist("app0:assets/parsers") then
	local path = "app0:assets/parsers/"
	...
	for i = 1, #files do
		...
		if not file.directory then
			...
			if not suc then
				Console.error("Can't load " .. path .. file.name .. ": " .. err)
			end
		end
	end
end

-- Load user-installed parsers from ux0:data/noboru/parsers/
