...
function InstallBundledParsers()
	local srcPath = "app0:assets/parsers/"
	if not doesDirExist(srcPath) then
		return
	end
	createDirectory("ux0:data/noboru/parsers")
	local files = listDirectory(srcPath) or {}
	for i = 1, #files do
		local f = files[i]
		if not f.directory and f.name:match("%.lua$") then
			copyFile(srcPath .. f.name, "ux0:data/noboru/parsers/" .. f.name)
		end
	end
end
InstallBundledParsers()
